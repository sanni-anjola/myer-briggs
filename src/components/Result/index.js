      ...
      <Content>
        <Personality>
          <h1 style={{ color: "black" }}>
            <a style={{ color: "inherit" }} href={`${personalities[personalityType].site}`} target="_blank" rel="noopener noreferrer">
              {personalityType}
            </a>
          </h1>
          <h3 style={{ color: "var(--semicolonGrey1)" }}>
            {personalities[personalityType].title}
          </h3>
          {personalities[personalityType].description.map((attr, index) => {
            return <h4 key={index}>{attr}</h4>;
          })}
        </Personality>
        <BreakDown>
          <h3>Amazing!</h3>
